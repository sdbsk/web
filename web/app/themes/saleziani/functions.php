...
add_action('enqueue_block_editor_assets', function () use ($manifest): void {
    ...
    foreach (get_cumstom_block_types() as $blockType) {
        wp_enqueue_script($blockType . '-block', get_template_directory_uri() . '/assets/private/blocks/' . $blockType . '.js', ['wp-blocks', 'wp-element']);
    }
});
...
function get_cumstom_block_types(): array
{
    ...
    foreach (scandir(TEMPLATEPATH . '/assets/private/blocks/') as $filename) {
